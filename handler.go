...
func call(ctx context.Context, functionName string, fn reflect.Value, r io.Reader) (ret interface{}, ee error) {
	defer func() {
		e := recover()
		if e == nil {
			return
...
//
// Collector holds functions for collecting metrics about function calls and other incoming HTTP requests. Collector may be nil.
//
...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		case "POST":
			...
			t0 := time.Now()
			r, xerr := call(r.Context(), name, fn, r.Body)
		...
		case "GET":
			...
			t0 := time.Now()
			r, xerr := call(r.Context(), name, fn, strings.NewReader(body))
			durationSec := float64(time.Now().Sub(t0)) / float64(time.Second)
			if xerr != nil {
				switch err := xerr.(type) {
				case *InternalServerError:
